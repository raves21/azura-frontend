...
import { DisableNewLine } from "@/utils/variables/tipTapCustomExtensions";
...
export default function EditProfilePage({
  ...
  const {
    ...
    setEditorContent: setUserNameInput
  } = useTipTapEditor({
    focusOnMount: false,
    placeholder: "eg. Jaylord Cabadingan",
    maxLength: 30,
    customExtensions: [DisableNewLine]
  });
      ...
      <div className="relative w-full h-48 shrink-0">
        <div className="absolute size-full">
          ...
          <div className="absolute flex gap-6 -translate-x-1/2 -translate-y-1/2 left-1/2 top-1/2">
            ...
            {banner && (
              <button className="transition-colors rounded-full hover:bg-gray-700/80 size-12 bg-socialPrimary/60 place-items-center">
        ...
        <div className="absolute -bottom-[38%] left-4 size-[120px] rounded-full  border-4 overflow-hidden box-content border-socialPrimary">
          ...
          <div className="absolute flex gap-3 -translate-x-1/2 -translate-y-1/2 left-1/2 top-1/2">
            <button className="transition-colors rounded-full hover:bg-gray-700/80 size-10 bg-socialPrimary/60 place-items-center">
              <ImageUp className="size-[55%] stroke-[1.5px]" />
            </button>
            {banner && (
              <button className="transition-colors rounded-full hover:bg-gray-700/80 size-10 bg-socialPrimary/60 place-items-center">
      ...
      <div className="flex flex-col gap-6 px-5 mb-8">
        <div className="flex flex-col w-full gap-2">
          ...
          <EditorContent
            ...
            className="rounded-md border-[0.5px] border-socialTextSecondary px-3 py-3"
          />
        </div>
        <div className="flex flex-col w-full gap-2">
          <div className="flex justify-between">
