...
export default function Unfollow({ type, isHovering }: UnfollowButtonProps) {
  if (type === "profilePage") {
    return (
      <div className="flex items-center self-end gap-2 px-4 py-2 font-semibold transition-colors border border-gray-600 rounded-full sm:px-5 lg:text-md hover:text-red-500 hover:border-red-500 lg:mt-2 sm:text-sm text-2xs mobile-m:text-xs hover:bg-red-500/20">
        {isHovering ? (
          <>
